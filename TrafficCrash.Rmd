---
title: "Road accident cause analysis and injury severity prediction with Machine Learning"
...
The purpose of this project is to predict road accident hotspots and analyse non-human factors causing accidents, using traffic crash data from 2011 to 2023 from the New Zealand governments open data portal (data.gov.nz). Here is research questions that the project is aiming to answer:
...
The next steps is to find whether they are any NAs and any outliers. Number of NA values were found to be very small and will be deleted. Box plot shows no outliers. It was also found that there are some labels that are missing. These missing labels were re-labelled as "Unknown".
...
## 7. EXPLORATORY DATA ANALYSIS & VISUALIZATIONS (Mithirendra/Yao Hong)

Next, an exploratory data analysis is conducted on the data to get a feel of the data and to gain more insights on the variables. Due to the sheer size of the dataset, the data has been further subset to only include observations from year 2015 onwards to year 2022.
...
The following analysis is to understand if accidents are causes by any non human variables. 
...
## 7. DATA MODELING & PREDICTIONS  (By Wei Ven/Ain)
...
3 different models were explored by our team to understand the peformance of each model. The best model will be chosen the models that were explored.
...
## 8. EVALUATION OF MODELS (By Wei Ven/Ain)

This section calculates the Root Mean Square Error (RMSE) for the 3 models.
...
## 9. DATA INTERPRETATION & DISCUSSION OF OUTPUT (By Wei Ven/Ain)
